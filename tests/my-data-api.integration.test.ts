...
describe("MY data admin API", () => {
  it("imports multiple CSV files, filters snapshots and protects clear endpoint", async () => {
    ...
    try {
      ...
      const enrichedHeader = `${header},发货模式,类目,评分,评论数`;
      const enrichedBody = multipart([{ name: "filtered.csv", content: `\uFEFF${enrichedHeader}\n1003,商品 C,0,30,900,1000,2,0,рюкзак,https://ozon.ru/p/1003,,local,2026-08-17T03:20:24Z,FBS,家居/收纳,4.7,321\n` }]);
      ...
      expect(filtered.json()).toMatchObject({ total: 1, items: [{ sku: "1003", category: "家居/收纳", fulfillmentMode: "FBS", rating: 4.7, reviewCount: 321 }] });
      expect(filtered.json().facets).toMatchObject({ categories: ["家居/收纳"], fulfillmentModes: expect.arrayContaining(["FBS", "unknown"]) });
      const ratingFiltered = await app.inject({ method: "GET", url: "/api/selection/my/products?captureDay=2026-08-17&minRating=4.5&maxReviewCount=500", cookies: { ozon_session: cookie } });
      expect(ratingFiltered.statusCode).toBe(200);
      expect(ratingFiltered.json()).toMatchObject({ total: 1, items: [{ sku: "1003", rating: 4.7, reviewCount: 321 }] });
