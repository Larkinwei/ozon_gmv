-- Allow a published product to remain auditable while its warehouse stock is
-- configured or verified separately.
CREATE TABLE resell_tasks_new (
  ...
  ozon_task_id TEXT,
  product_id TEXT,
  status TEXT NOT NULL CHECK (status IN ('draft', 'preflight_failed', 'creating', 'pending', 'created', 'setting_images', 'setting_price', 'setting_stock', 'stock_pending', 'moderating', 'sellable', 'needs_input', 'failed')),
  last_error TEXT,
  created_at_ms INTEGER NOT NULL,
  updated_at_ms INTEGER NOT NULL,
  completed_at_ms INTEGER,
  source_type TEXT NOT NULL DEFAULT 'follow_sell',
...
INSERT INTO resell_tasks_new (
  ...
  ozon_task_id, product_id, status, last_error, created_at_ms, updated_at_ms,
  completed_at_ms, source_type, source_snapshot_json, idempotency_key
)
SELECT
  id, store_id, source_sku, target_offer_id, mode, price, old_price, currency, vat,
  stock, fulfillment_mode, warehouse_id, title, description, attributes_json,
