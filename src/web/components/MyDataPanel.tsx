...
export function MyDataPanel(props: MyDataPanelProps): React.JSX.Element {
  ...
  useEffect(() => {
    if (!captureDay && overviewQuery.data?.latestCaptureDay) {
      setCaptureDay(overviewQuery.data.latestCaptureDay);
    }
  }, [captureDay, overviewQuery.data?.latestCaptureDay]);
  ...
  const clearMutation = useMutation({
    ...
    onSuccess: async () => {
      setCaptureDay("");
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ["my-data-overview"] }),
        queryClient.invalidateQueries({ queryKey: ["my-data-products"] }),
      ]);
  ...
  const hasData = (overviewQuery.data?.productCount ?? 0) > 0;
  return (
    <section className="selection-tab-panel my-data-panel" role="tabpanel" aria-label="MY 数据">
      ...
      {overviewQuery.isLoading ? <div className="my-data-skeleton" aria-busy="true">正在读取 MY 数据…</div> : (
        <>
          <div className="my-data-kpis">
            <Metric label="当前商品数" value={(overviewQuery.data?.productCount ?? 0).toLocaleString("zh-CN")} hint={captureDay || "尚未导入"} />
          ...
          <div className="my-data-toolbar">
            ...
            <label className="my-data-field"><span>采集日期</span><select value={captureDay} onChange={(event) => { setCaptureDay(event.target.value); setPage(1); }}><option value="">选择日期</option>{(productsQuery.data?.captureDays ?? overviewQuery.data?.captureDays ?? []).map((day) => <option value={day} key={day}>{day}</option>)}</select></label>
      ...
      {showImport && <MyDataImportDialog onClose={() => setShowImport(false)} onImported={async (result) => { setShowImport(false); setCaptureDay(result.captureDays.at(-1) ?? ""); await Promise.all([queryClient.invalidateQueries({ queryKey: ["my-data-overview"] }), queryClient.invalidateQueries({ queryKey: ["my-data-products"] })]); props.onNotice({ tone: "success", text: `MY 数据导入完成：新增 ${result.importedFiles} 个文件，${result.validRows.toLocaleString("zh-CN")} 条有效记录，跳过 ${result.duplicateFiles} 个重复文件。` }); }} />}
    </section>
  );
}
...
function MyDataImportDialog(props: { onClose: () => void; onImported: (result: MyDataImportResult) => Promise<void> }): React.JSX.Element {
  ...
  async function chooseFolder(event: React.ChangeEvent<HTMLInputElement>): Promise<void> {
    const selected = Array.from(event.target.files ?? []).filter((file) => file.name.toLowerCase().endsWith(".csv"));
    if (selected.length === 0) {
      setError("所选文件夹中没有 CSV 文件。");
      return;
    }
